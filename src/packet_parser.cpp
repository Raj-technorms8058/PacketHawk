...
bool PacketParser::parseTCP(const uint8_t* data, size_t len, ParsedPacket& parsed) {
    ...
    parsed.flag_fin = data[13] & 0x01;
    parsed.flag_syn = data[13] & 0x02;
    parsed.flag_rst = data[13] & 0x04;
    parsed.flag_psh = data[13] & 0x08;
    parsed.flag_ack = data[13] & 0x10;

    if (len > header_len) {
        parsed.payload.assign(data + header_len, data + len);
